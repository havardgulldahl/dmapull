...
function createQhtml() {
    ...
    for(var i=0; i<Q.length;i++) {
        var t = getMetadata(Q[i]).title || "Song #"+(i+1).toString();
    ...
    e.on("click", function(ev) {
        ...
        switch(btn.data("cmd")) {
            ...
            case "play":
                chrome.runtime.sendMessage({"msg":"play", "item":Q[_id]}); break;
            case "pause":
                chrome.runtime.sendMessage({"msg":"pause", "item":Q[_id]}); break;
            case "download":
                chrome.runtime.sendMessage({"msg":"download", "item":Q[_id]}); break;
        }
    });
...
jQuery(document).ready(function() {
    console.log("popup");
    chrome.runtime.sendMessage({msg:"rawQ"}, function(response) {
